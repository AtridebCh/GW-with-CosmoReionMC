    ...
    Type TThermoData
        ...
        !rpc
        real(dl), dimension(:), allocatable :: xe_old
        !end_rpc
...
! Limit how small opacity can be to avoid divergence of 1/opacity
          if (abs(opacity) < 1.d-8) opacity = sign(1.d-8,opacity)
!END RPC
...
!  for the definition of fe to avoid diverging quantities.
          if (this%xe(i-1)<=((1._dl-CP%yhe/2.d0)/(1._dl-CP%yhe)).and.this%xe(i-1)>=0.) then
             fe=(1._dl-CP%yhe)*this%xe(i-1)/(1._dl-0.75d0*CP%yhe+(1._dl-CP%yhe)*this%xe(i-1))
          else if (this%xe(i-1)<0.) then
             fe=1.d-4
          else
             fe=(1._dl-CP%yhe)*((1._dl-CP%yhe/2.d0)/(1._dl-CP%yhe))/(1._dl-0.75d0*CP%yhe+ &
             (1._dl-CP%yhe)*((1._dl-CP%yhe/2.d0)/(1._dl-CP%yhe)))
          end if
...
!RPC
               if (last_dotmu /=0 .and. this%dotmu(i) /= 0 .and. &
                     abs(last_dotmu+this%dotmu(i)) > abs(last_dotmu-this%dotmu(i)) ) then
...
!end_rpc
        !write(*,*)this%xe(i),xe_a(i),tau,State%reion_tau_start
...
!          if (tight_tau==0 .and. 1/(tau*dotmu(i)) > 0.005) tight_tau = tau !0.005
          if (this%tight_tau==0 .and. 1/(tau*abs(this%dotmu(i))) > 0.005) this%tight_tau = tau !0.005
!END RPC
