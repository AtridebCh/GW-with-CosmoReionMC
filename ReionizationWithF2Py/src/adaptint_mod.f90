...
CONTAINS
  ...
  subroutine simpson_integrate(f, n, dx, res)
    implicit none

    integer,  intent(in)  :: n              ! number of intervals, must be even
    real(dp), intent(in)  :: f(0:n)         ! function values at n+1 points
    real(dp), intent(in)  :: dx             ! uniform step size
    real(dp), intent(out) :: res
    integer :: i

    if (mod(n, 2) /= 0) stop 'simpson_integrate: n must be even'

    res = f(0) + f(n)   ! endpoints

    do i = 1, n - 1
      if (mod(i, 2) == 1) then
        res = res + 4.0_dp * f(i)   ! odd
      else
        res = res + 2.0_dp * f(i)   ! even
      end if
    end do
